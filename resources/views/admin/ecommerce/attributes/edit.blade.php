...
@section('content')
    ...
    <div class="row user">       
        <div class="col-md-12">
            ...
            @foreach($productattributedtl as $productattribute)
            <form action="{{ route('admin.product-attributes.update',['id'=>$productattribute->id]) }}" method="POST" role="form" enctype="multipart/form-data">
                @csrf                
                <div class="tile">  
                    <div class="row">
                        <div class="form-group col-md-8">
                            <label class="control-label" for="site_name"> Name</label>
                            <input class="form-control" type="text" placeholder="Enter  name" id="name" name="name" value="{{ old('name', $productattribute->name) }}"/>
                            @error('name')
                        ...
                        <div class="form-group col-md-8">
                            ...
                            <input class="form-control" type="text" placeholder="Enter Code" id="code" name="code" value="{{ old('code', $productattribute->code) }}"/>
                            @error('code')
                                <div class="alert alert-danger error">{{ $message }}</div>
                            @enderror
                        </div>                                          
