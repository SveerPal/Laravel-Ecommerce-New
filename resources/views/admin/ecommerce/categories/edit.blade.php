...
@section('content')
    ...
    <div class="row user">       
        <div class="col-md-12">
            ...
            @foreach($productcategoriesdtl as $productcategory)
            <form action="{{ route('admin.product-categories.update',['id'=>$productcategory->id]) }}" method="POST" role="form" enctype="multipart/form-data">
                @csrf                
                <div class="tile">  
                    <div class="row">
                        <div class="form-group col-md-6">
                            <label class="control-label" for="site_name"> Name</label>
                            <input class="form-control" type="text" placeholder="Enter  name" id="name" name="name" value="{{ old('name', $productcategory->name) }}"/>
                            @error('name')
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input class="form-control" type="text" placeholder="Enter Slug" id="slug" name="slug" value="{{ old('slug', $productcategory->slug) }}"/>
                            @error('slug')
                                <div class="alert alert-danger error">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" id="parent" name="parent" value="{{ old('parent') }}"/>
                                ...
                                @foreach($parentproductcategories as $parentproductcategories)                                
                                    <option @if ($parentproductcategories->id==$productcategory->parent) selected="selected" @endif value="{{ $parentproductcategories->id }}">{{ $parentproductcategories->name }}</option>
                                @endforeach 
                            </select>
                            @error('parent')
                                <div class="alert alert-danger error">{{ $message }}</div>
                        ...
                        <div class="form-group col-md-5">
                            ...
                            <input class="form-control" type="hidden" name="banner_old" value="{{ $productcategory->banner }}"/>
                            @error('banner')
                                <div class="alert alert-danger error">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input class="form-control" type="text" placeholder="Enter  alt" id="alt" name="alt" value="{{ old('alt', $productcategory->alt) }}"/>
                            @error('alt')
                                <div class="alert alert-danger error">{{ $message }}</div>
                            @enderror                                  
                        </div>
                        <div class="form-group col-md-6">
                            ...
                            <textarea class="form-control" rows="1" placeholder="Enter Meta Title" id="meta_title" name="meta_title">{{ old('meta_title', $productcategory->meta_title) }}</textarea>
                            @error('meta_title')
                                <div class="alert alert-danger error">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group col-md-12">
                            ...
                            <textarea class="form-control" rows="4" placeholder="Enter seo meta description for store" id="meta_description"
                                name="meta_description" >{{ old('meta_description', $productcategory->meta_description) }}</textarea>
